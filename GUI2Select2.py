...
# 通过调用 matplotlib.use('TkAgg')，来告诉 Matplotlib 使用 TkAgg 后端作为图形显示的方式。
# TkAgg 使用 Tkinter 来创建一个 Tkinter 窗口，并在其中显示图形。Tkinter 是 Python 的标准图形用户界面（GUI）工具包，它提供了创建简单的图形界面的功能。
...
# Create a button for all states
all_button = tk.Button(root, text='ALL STATES', command=lambda: plot_state_data('ALL STATES'))
all_button.grid(row=0, column=0, columnspan=2)  # Spanning the button across 2 columns

rows = len(states) // 2  # Number of rows for states buttons (considering 2 columns)

# Create a button for each state
for i, state in enumerate(states):
    button = tk.Button(root, text=state, command=lambda s=state: plot_state_data(s))
    button.grid(row=1 + (i // 2), column=i % 2)  # Distributing buttons in 2 columns
