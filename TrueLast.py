...
def forecast(df_pivot, states):
    ...
    for state in states:
        ...
        early_stopping = EarlyStopping(monitor='loss', patience=5, verbose=0)  # 静默输出进度
...
def plot_state_data(state_name, ax, fig, actual_lines_dict, forecast_lines_dict, parent):
    ...
    if state_name in actual_lines_dict and state_name in forecast_lines_dict:
        ...
        # Plot actual data
        ax.plot(actual_x, actual_y, label=state_name + ' (Actual data from 2021-4-1 to 2022-11-1)')

        # Plot forecast data
        ax.plot(forecast_x, forecast_y, ':', label=state_name + ' (Forecast from 2021-10-15 following 15 days)')
        ...
        ax.set_ylabel('Cases')
        ax.legend()
        y_values = actual_y.astype(float)  # <class 'pandas.core.series.Series'> y
        x_values = actual_x.astype(float)  # <class 'pandas.core.indexes.datetimes.DatetimeIndex'> x
        yf_values = forecast_y.values.astype(float)
        xf_values = forecast_x.values.astype(float)
        # Calculate and display slope difference
        actual_slope = (y_values[-1] - y_values[-14]) / (x_values[-1] - x_values[-14])
        forecast_slope = (yf_values[-1] - yf_values[-14]) / (xf_values[-1] - xf_values[-14])
        slope_difference = actual_slope - forecast_slope
        slope_text = f"Slope Difference: {slope_difference:.2f}"
        ax.text(0.95, 0.95, slope_text, horizontalalignment='right', verticalalignment='top',
